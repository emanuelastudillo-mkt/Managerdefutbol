...
(function(){
  ...
  function pcGroupPlayerCard(player,entry){
    ...
    return `<button class="tactic-group-player" data-player-id="${player.id}">${typeof faceImg==='function'?faceImg(player,'drag-face'):''}<span><strong>${escapeHtml(playerLastName(player.name||''))}</strong><small>${escapeHtml(player.position||'')} · ${typeof visibleOverall==='function'?visibleOverall(player):player.overall||0} · Conf. ${Math.round(Number(entry?.value||50))}</small>${tags.length?`<em>${escapeHtml(tags.join(' · '))}</em>`:''}</span></button>`;
  }
