...
function leagueEconomyDivisionIdForMatch(match){
  if(match?.clubWorldCup || match?.libertadores || String(match?.divisionId || '') === 'club-world-cup') return '';
...
function applyEconomyResult(match){
  ...
  if(match?.clubWorldCup || match?.nationalCup || (match?.libertadores && match?.neutralVenue) || String(match?.divisionId || '') === 'club-world-cup') return 0;
